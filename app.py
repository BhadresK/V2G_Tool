...
import numpy as np
import pandas as pd
from grid_page import render_grid_page
...
from grid_sim import background_total_kw
_P_BG_FULL: np.ndarray = background_total_kw()

# German grid tariff on top of the SMARD spot price, all in ct/kWh. These are the current (2025) pass-through fees a depot pays regardless of V2G - network fee is the big one at 6.63 ct. Update this dict if the Netzentgelt changes for the next tariff year.
...
# how many ct/kWh of the levies above V2G discharge currently gets to skip. 0 for today (full double taxation - depot pays levies again on the exported energy). Future A/B are the two exemption scenarios from EnWG §118 Abs.6 discussed in the thesis - see the "Regulatory scenarios" caption below.
...
FUTURE_A_EXEMPT_CT = 6.63                                       # Netzentgelt only
FUTURE_B_EXEMPT_CT = 6.63 + 2.05 + 0.446 + 0.941 + 1.559        # everything except Konzessionsabgabe
...
def parse_hhmm(s: str, default: float) -> float:
    '''
    Falls back to `default` on anything malfunction (empty string, "25:00", letters, ...) instead of raising - this is on purpose so a bad text_input value never crashes the whole page, but it does mean only a typo in the arrival/departure box fails.
    '''
    try:
        parts = s.strip().split(":")
        h = int(parts[0])
...
def _alt_x(hours_d, is_48h, is_wknd_fullday, is_long_weekend=False):
    # builds the Altair for one of 4 chart
...
def _plug_rects(hours_d, plug_d, dt):
    ...
    # Scans plug_d and merges consecutive "plugged in" slots into rectangles for the gold background band.
...
def _vrow_data(arrival_h, departure_h, is_48h, is_wknd_fullday,
    ...
    # weekday charts run noon-to-noon, so any clock hour before noon actually falls on the "next day" of the chart
...
def make_power_chart(v2g, hours_d, buy_d, plug_d,
    ...
    # To draw the real 7.6/0.7 kW (or 9.7/0.65/0 kW) on/off cycle at minute resolution for the chart.
    n_win = len(hours_d)
    tru_min_precise = np.zeros(n_win * 60)
    if tru_cycle and tru_cycle != "OFF" and tru_d is not None and n_win > 0:
        tru_arr_np    = np.asarray(tru_d, dtype=float)[:n_win]
        active_hourly = tru_arr_np > 1e-9
        if np.any(active_hourly):
            active_start = int(np.argmax(active_hourly))
            active_end   = n_win - int(np.argmax(active_hourly[::-1]))
            n_active     = active_end - active_start

            tru_hourly_cycle_active = get_tru_1h_trace(tru_cycle, n_active, 1.0)
            parasitic = float(np.mean(
                tru_arr_np[active_start:active_end] - tru_hourly_cycle_active))

            tru_min_cycle_active = get_tru_1h_trace(tru_cycle, n_active * 60, 1.0 / 60.0)
    ...
    def expand_arr(P_c, P_d, tru_arr):
        ...
        if peak_shaving:
            # peak-shaving mode: the MILP already spreads charge power flat across each hour to hit the peak target, so just repeat the hourly value 60x.
            for i in range(n):
                pc_val = _pc(i)
                pd_val = _pd(i)
                for m in range(60):
            ...
            # normal mode: the MILP only gives us one charge/discharge kW per hour
            i = 0
            while i < n:
                if _pc(i) > 1e-6:
                    j = i
...
def make_soc_chart(v2g, hours_d, plug_d,
    ...
    def _minsoc(result):
        ...
        t_m, _, _, soc_p = expand_to_minutes(v2g, np.asarray(Pc), np.asarray(Pd), E0, tru,
                                              peak_shaving=peak_shaving, tru_cycle=tru_cycle)
    ...
    chart = (
        alt.layer(*layers)
        .properties(title=f'SoC : Dumb vs {x_label}', height=300, background='#FAFAFA')
        .interactive()
    )
    return chart
...
def load_two_day_profile(date_str: str) -> np.ndarray:
    # combines [selected day] + [next day] so a weekday overnight window has real prices on both sides of midnight.
...
def load_seasonal_longweekend_profile(months: tuple, arrival_h: float, departure_h: float):
    ...
    # Fri afternoon -> Sat -> Sun -> Mon morning
...
def _apply_mpc_distortions(buy_arr, reduce_night, reduce_pct,
    ...
    # deliberately mangles the price curve to stress-test scenario D (MPC) against forecast error
    ...
    for i in range(len(p)):
        ...
        if increase_eve and 16 <= h <= 21:
            p[i] *= max(0.0, 1.0 - increase_pct / 100.0)
        for sp in spikes:
            if h == int(sp[0]) % 24:
                p[i] *= float(sp[1])
    return p
...
def run_seasonal(season_key, arrival_h, departure_h,
                 ...
                 price_spikes=(), peak_shaving=False):
    ...
    if is_48h:
        ...
        v2gp_long_mpc = compose_v2gp_price(buy_long_mpc, exempt_ct=0.0)
        W     = len(buy_long)
        ...
        _ps      = peak_shaving
        _bg_full_48 = np.tile(_P_BG_FULL, int(np.ceil(W / 24)))[:W]

        results = []

        Pc, Pd, soc = run_A_dumb(v2g, buy_long_mpc, v2gp_long_mpc, W, E_init, tru_w)
        kpi_A = make_kpi("A - Dumb", v2g, Pc, Pd, soc,
                         buy_long_mpc, v2gp_long_mpc, E_init,
                         is_weekend_48=True, tru_w=tru_w, P_bg_full=_bg_full_48)
        results.append(kpi_A)
        _dumb_peak_48 = kpi_A["peak_kw"]

        if do_B:
            Pc, Pd, soc = run_B_smart(v2g, buy_long, v2gp_long, E_init, tru_w,
                                      peak_shaving=_ps, P_bg_w=_bg_full_48,
                                      P_bg_full=_bg_full_48)
            results.append(make_kpi("B - Smart (no V2G)", v2g, Pc, Pd, soc,
                                    buy_long_mpc, v2gp_long_mpc, E_init,
                                    is_weekend_48=True, tru_w=tru_w,
                                    P_bg_full=_bg_full_48, dumb_peak_kw=_dumb_peak_48))
        if do_C:
            ...
            results.append(make_kpi("C - MILP Day-Ahead", v2g, Pc, Pd, soc,
                                    ...
                                    P_bg_full=_bg_full_48, dumb_peak_kw=_dumb_peak_48))
        if do_D:
            Pc, Pd, soc = run_D_mpc(v2g, buy_long_mpc, v2gp_long_mpc, E_init, tru_w,
                                    peak_shaving=_ps, P_bg_w=_bg_full_48,
                                    P_bg_full=_bg_full_48)
    ...
    # The sidebar deviation sliders moved the actual arrival/departure away from the planned ones
    ...
    _ps       = peak_shaving
    _bg_w     = _P_BG_FULL[win_act]   # background kW during actual plug-in window
    _bg_full  = _P_BG_FULL            # full 24h background

    # A
    Pc, Pd, soc = run_A_dumb(v2g, buy_w_act_mpc, v2gp_w_act_mpc, W_act, E_init, tru_w_act)
    kpi_A = make_kpi("A - Dumb", v2g, Pc, Pd, soc,
                     buy_w_act_mpc, v2gp_w_act_mpc, E_init,
                     arr_act, dep_act, tru_w=tru_w_act, P_bg_full=_bg_full)
    results.append(kpi_A)
    _dumb_peak = kpi_A["peak_kw"]

    # B
    if do_B:
        Pc, Pd, soc = run_B_smart(v2g, buy_w_plan, v2gp_w_plan, E_init, tru_w_plan,
                                  peak_shaving=_ps, P_bg_w=_P_BG_FULL[win_plan],
                                  P_bg_full=_bg_full)
        if abnormal:
        ...
        results.append(make_kpi("B - Smart (no V2G)", v2g, Pc, Pd, soc,
                                ...
                                P_bg_full=_bg_full, dumb_peak_kw=_dumb_peak))
    ...
    if do_D:
        Pc, Pd, soc = run_D_mpc(v2g, buy_w_act_mpc, v2gp_w_act_mpc, E_init, tru_w_act,
                                 peak_shaving=_ps, P_bg_w=_bg_w, P_bg_full=_bg_full)
        results.append(make_kpi("D - MPC (receding)", v2g, Pc, Pd, soc,
                                buy_w_act_mpc, v2gp_w_act_mpc, E_init,
                                arr_act, dep_act, tru_w=tru_w_act,
                                P_bg_full=_bg_full, dumb_peak_kw=_dumb_peak))
...
def run_specific_date(date_str, arrival_h, departure_h,
    ...
    # if Friday -> treated as a Fri-arrival/Mon-departure long weekend, Sat/Sun -> a plain 24h full-day-plugged-in block, anything else -> the normal overnight weekday window.
    v2g    = V2GParams(soc_departure_pct=soc_departure_pct)
    E_init = v2g.usable_capacity_kWh * soc_pct / 100.0
    _aux_kw       = aux_power_w / 1000.0
    ...
    if is_friday:
        ...
        _ps         = peak_shaving
        _bg_full_lw = np.tile(_P_BG_FULL, int(np.ceil(W / 24)))[:W]

        results = []
        Pc, Pd, soc = run_A_dumb(v2g, buy_long_mpc, v2gp_long_mpc, W, E_init, tru_w)
        kpi_A = make_kpi("A - Dumb", v2g, Pc, Pd, soc,
                         buy_long_mpc, v2gp_long_mpc, E_init, is_weekend_48=True,
                         tru_w=tru_w, P_bg_full=_bg_full_lw)
        results.append(kpi_A)
        _dumb_peak_lw = kpi_A["peak_kw"]

        if do_B:
            Pc, Pd, soc = run_B_smart(v2g, buy_long, v2gp_long, E_init, tru_w,
                                      peak_shaving=_ps, P_bg_w=_bg_full_lw,
                                      P_bg_full=_bg_full_lw)
            results.append(make_kpi("B - Smart (no V2G)", v2g, Pc, Pd, soc,
                                    buy_long_mpc, v2gp_long_mpc, E_init, is_weekend_48=True,
                                    tru_w=tru_w, P_bg_full=_bg_full_lw,
                                    dumb_peak_kw=_dumb_peak_lw))
        if do_C:
            Pc, Pd, soc = run_C_milp(v2g, buy_long, v2gp_long, E_init, tru_w,
                                     peak_shaving=_ps, P_bg_w=_bg_full_lw,
                                     P_bg_full=_bg_full_lw)
    ...
    if is_wknd:
        ...
        buy_d = _apply_mpc_distortions(buy, reduce_night, reduce_night_pct,
                                       increase_eve, increase_eve_pct, price_spikes)
        v2gp_d = compose_v2gp_price(buy_d, exempt_ct=0.0)
        plug_d = np.ones(24)
        ...
        _ps = peak_shaving
        _bg_full_wd = _P_BG_FULL

        results = []

        Pc, Pd, soc = run_A_dumb(v2g, buy_d, v2gp_d, W, E_init, tru_w)
        kpi_A = make_kpi("A - Dumb", v2g, Pc, Pd, soc,
                         buy_d, v2gp_d, E_init, arr, dep,
                         tru_w=tru_w, P_bg_full=_bg_full_wd)
        results.append(kpi_A)
        _dumb_peak_wd = kpi_A["peak_kw"]

        if do_B:
            Pc, Pd, soc = run_B_smart(v2g, buy_w, v2gp_w, E_init, tru_w,
                                      peak_shaving=_ps, P_bg_w=_bg_full_wd,
                                      P_bg_full=_bg_full_wd)
            results.append(make_kpi("B - Smart (no V2G)", v2g, Pc, Pd, soc,
                                    buy_d, v2gp_d, E_init, arr, dep,
                                    tru_w=tru_w, P_bg_full=_bg_full_wd,
                                    dumb_peak_kw=_dumb_peak_wd))
        ...
        if do_D:
            Pc, Pd, soc = run_D_mpc(v2g, buy_d, v2gp_d, E_init, tru_w,
                                    peak_shaving=_ps, P_bg_w=_bg_full_wd,
                                    P_bg_full=_bg_full_wd)
            results.append(make_kpi("D - MPC (receding)", v2g, Pc, Pd, soc,
                                    buy_d, v2gp_d, E_init, arr, dep,
    ...
    def _slots_48(arr_h, dep_h):
        ...
        if d <= a:
            return np.array(list(range(a, 24)) + list(range(24, 24 + d)))
        return np.array(list(range(a, d)))
    ...
    _ps      = peak_shaving
    _bg_full = _P_BG_FULL                     
    _bg_w_plan = _P_BG_FULL[slots_plan % 24]  
    _bg_w_act  = _P_BG_FULL[slots_act  % 24]  

    results = []

    # A
    Pc, Pd, soc = run_A_dumb(v2g, buy_w_act_mpc, v2gp_w_act_mpc, len(slots_act), E_init, tru_w_act)
    kpi_A = make_kpi("A - Dumb", v2g, Pc, Pd, soc,
                     buy_w_act_mpc, v2gp_w_act_mpc, E_init,
                     arr, dep, tru_w=tru_w_act, P_bg_full=_bg_full)
    results.append(kpi_A)
    _dumb_peak = kpi_A["peak_kw"]
    ...
    if do_B:
        Pc, Pd, soc = run_B_smart(v2g, buy_w_plan, v2gp_w_plan, E_init, tru_w_plan,
                                  peak_shaving=_ps, P_bg_w=_bg_w_plan,
                                  P_bg_full=_bg_full)
        if abnormal:
            Pc, Pd, soc = realize_planned_window_under_actual_times(
                v2g, Pc, Pd, E_init,
    ...
    if do_D:
        Pc, Pd, soc = run_D_mpc(v2g, buy_w_act_mpc, v2gp_w_act_mpc, E_init, tru_w_act,
                                peak_shaving=_ps, P_bg_w=_bg_w_act,
                                P_bg_full=_bg_full)
        results.append(make_kpi("D - MPC (receding)", v2g, Pc, Pd, soc,
                                buy_w_act_mpc, v2gp_w_act_mpc, E_init,
                                arr, dep, tru_w=tru_w_act,
...
DEFAULTS = {
    ...
    "price_reduce_night_pct": 0,
    "price_increase_eve": False,
    "price_increase_eve_pct": 0,
    "arrival_dev_h": 0.0,
    "departure_dev_h": 0.0,
    "aux_power_w": 400,
    "bat_heat_w":  100,
...
def render_input_panel():
    ...
    st.caption("Master's Thesis | TU Dortmund | Schmitz Cargobull AG")
    st.markdown("---")
    ...
    with st.form("input_form", clear_on_submit=False):
        ...
        with c1:
            ...
            step=1.0,
            help="Positive = later arrival, negative = earlier arrival."
            )
        ...
        with c2:
            ...
            st.markdown("##### Price Distortions")
            cfg["price_reduce_night"] = st.checkbox(
                "Increase 00:00–05:00 prices", bool(cfg.get("price_reduce_night", False)), key="form_rn")
            cfg["price_reduce_night_pct"] = st.number_input(
                "Increase %", 0, 100,
                max(0, min(100, int(cfg.get("price_reduce_night_pct", 0)))), key="form_rn_pct")
            cfg["price_increase_eve"] = st.checkbox(
                "Decrease 16:00–22:00 prices", bool(cfg.get("price_increase_eve", False)), key="form_ie")
            cfg["price_increase_eve_pct"] = st.number_input(
                "Decrease %", 0, 100,
                max(0, min(100, int(cfg.get("price_increase_eve_pct", 0)))), key="form_ie_pct")
        ...
        with c3:
            ...
            cfg["bat_heat_w"] = st.number_input(
                ...
                help="Constant power to keep battery warm in winter months. ")
        ...
        with c4:
            ...
            cfg["peak_shaving"] = st.toggle(
                "Enable depot peak shaving in MILP",
                value=bool(cfg.get("peak_shaving", False)),
                help=(
                    "Adds a peak-penalty term to the MILP objective "
                ),
            )
            st.markdown("---")
            st.markdown("##### Regulatory scenarios")
            st.caption(
                "**Current :** All 6 levies apply - full double taxation.\n\n"
        ...
        with st.expander(" System Parameters & Pre-defined Values", expanded=False):
            ...
            with col_sp1:
                ...
                st.caption("Usable capacity: **63 kWh**")
                ...
                st.caption("Min charge power: **2.0 kW**")
...
**Scenario A - Dumb:** Greedy charge. No price awareness.

**Scenario B - Smart:** Schedules charging at cheapest hours while meeting departure SoC. No Bidirectional.

**Scenario C - MILP Day-Ahead:** Full MILP with V2G. Optimises against price foresight.
...
_vat_rate      = VAT_RATE
_v2g_double_ct = V2G_RECOVERABLE_CURRENT_CT   
_vat_fut_rate  = VAT_RATE
...
with st.sidebar:
    ...
    _page = st.radio(
        "Choose view",
        ["Optimisation Results", "Grid Impact"],
        index=0,
        key="_page_select",
    )
    st.markdown("---")
    ...
    cfg["bat_heat_w"] = st.number_input(
        "Battery heating (W, winter)", min_value=0, max_value=1000,
        value=int(cfg.get("bat_heat_w", 100)), step=10, key="sb_heat")

    st.markdown("##### Price Distortions")
    cfg["price_reduce_night"] = st.checkbox(
        "Increase 00:00–05:00 prices", bool(cfg.get("price_reduce_night", False)), key="sb_rn")
    cfg["price_reduce_night_pct"] = st.number_input(
        "Increase %", 0, 100,
        max(0, min(100, int(cfg.get("price_reduce_night_pct", 0)))), key="sb_rn_pct")
    cfg["price_increase_eve"] = st.checkbox(
        "Decrease 16:00–22:00 prices", bool(cfg.get("price_increase_eve", False)), key="sb_ie")
    cfg["price_increase_eve_pct"] = st.number_input(
        "Decrease %", 0, 100,
        max(0, min(100, int(cfg.get("price_increase_eve_pct", 0)))), key="sb_ie_pct")

    # This tuple is a manual cache-relevant change checker: if it differs from last run, nuke the whole cache and force a rerun.
    _watch = (cfg["arrival_dev_h"], cfg["departure_dev_h"],
              cfg["aux_power_w"], cfg["bat_heat_w"],
              cfg["price_reduce_night"], cfg["price_reduce_night_pct"],
              cfg["price_increase_eve"], cfg["price_increase_eve_pct"])
...
if st.session_state.get("_page_select") == "Grid Impact":
    render_grid_page()
    st.stop()
...
with st.spinner("Loading price data..."):
    try:
        # n_days here is only used for the loading spinner context, not for any actual calculation
...
def show_kpi_table(results, fixed_price, tru_cycle, rc, label="", buy_d=None, total_stay=False):
    ...
    def _charge_cost_allin(r, exempt_ct):
        # this helper render all three regulatory columns of the KPI table.
        ...
        return (r["charge_cost"] * vat_gross
                + (non_exempt_kwh * _fixed_net_ct
                   + exempt_kwh * (_fixed_net_ct - exempt_ct)) / 100.0 * vat_gross)

    def _build_row(r, exempt_ct, is_fixed=False):
        if is_fixed:
            charge   = ref_A["charge_kwh"] * fixed_price
            v2g_rev  = 0.0
            deg_cost = ref_A.get("deg_cost_eur", 0.0)
        else:
        ...
        net_w_deg  = net_no_deg + deg_cost
        return charge, v2g_rev, deg_cost, net_no_deg, net_w_deg
    ...
    for i, sc_label in enumerate(scenario_labels):
        ...
        c_cur, v_cur, d_cur, n_cur, nwd_cur = _build_row(r, 0.0,                is_fixed)
        c_fa,  v_fa,  d_fa,  n_fa,  nwd_fa  = _build_row(r, FUTURE_A_EXEMPT_CT, is_fixed)
        c_fb,  v_fb,  d_fb,  n_fb,  nwd_fb  = _build_row(r, FUTURE_B_EXEMPT_CT, is_fixed)
        rows.append((sc_label,
                     c_cur, v_cur, d_cur, n_cur, nwd_cur,
                     c_fa,  v_fa,  d_fa,  n_fa,  nwd_fa,
                     c_fb,  v_fb,  d_fb,  n_fb,  nwd_fb))
    ...
    for i, (sc_label,
             c_cur, v_cur, d_cur, n_cur, nwd_cur,
             c_fa,  v_fa,  d_fa,  n_fa,  nwd_fa,
             c_fb,  v_fb,  d_fb,  n_fb,  nwd_fb) in enumerate(rows):
        tbody_rows += (
            f"<tr>"
            f"<td style='text-align:left;font-weight:600;white-space:nowrap'>{sc_label}</td>"
            f"<td>{c_cur:.3f}</td><td>{v_cur:.3f}</td><td>{d_cur:.3f}</td>"
            f"<td>{n_cur:.3f}</td><td style='font-weight:600'>{nwd_cur:.3f}</td>"
            f"<td>{c_fa:.3f}</td><td>{v_fa:.3f}</td><td>{d_fa:.3f}</td>"
            f"<td>{n_fa:.3f}</td><td style='font-weight:600'>{nwd_fa:.3f}</td>"
            f"<td>{c_fb:.3f}</td><td>{v_fb:.3f}</td><td>{d_fb:.3f}</td>"
            f"<td>{n_fb:.3f}</td><td style='font-weight:600'>{nwd_fb:.3f}</td>"
            f"</tr>"
        )
...
<thead>
  <tr>
    <th rowspan='2' style='text-align:center'>Scenario</th>
        ...
        try:
            (results, buy_d, plug_d, hours_d,
             is_wknd_r, is_48h, is_wknd_fullday, tru_d, rc, is_lw_sd) = run_specific_date(
                ...
                peak_shaving=bool(cfg.get("peak_shaving", False)),
            )
        except Exception as e:
            st.error(f"Error: {e}")
            st.stop()
